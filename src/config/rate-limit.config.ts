...
export const rateLimitConfig = (configService: ConfigService) => ({
    ...
    auth: {
        ttl: configService.get<number>('RATE_LIMIT_AUTH_TTL'),   // 1 MINUTE WINDOW
        limit: configService.get<number>('RATE_LIMIT_AUTH_LIMIT') // ONLY 5 REQUESTS ALLOWED (LOGIN PROTECTION)
    },

    sessions: {
        ttl: configService.get<number>('RATE_LIMIT_SESSIONS_TTL'),   // 1 MINUTE WINDOW
        limit: configService.get<number>('RATE_LIMIT_SESSIONS_LIMIT') // UP TO 100 REQUESTS PER MINUTE
    },
